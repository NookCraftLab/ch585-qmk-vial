...
__attribute__ ((aligned(4))) uint8_t USBHS_EP3_TX_Buf[ DEF_USB_EP3_HS_SIZE ];    //ep3_in(512)
__attribute__ ((aligned(4))) uint8_t USBHS_EP3_RX_Buf[ DEF_USB_EP3_HS_SIZE ];    //ep3_out(512)
    ...
    if( intflag & USBHS_UDIF_TRANSFER )
    {
        ...
        if( !(intst & USBHS_UDIS_EP_DIR )) // SETUP/OUT Transaction
        {
            switch( endp_num )
            {
                            ...
                            if (( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS)
                              {
                                ...
                                switch( USBHS_SetupReqCode )
                                {
                                    ...
                                    case HID_SET_IDLE:
                                        ...
                                        {
                                            USBHS_HidIdle[ 2 ] = (uint8_t)( USBHS_SetupReqValue >> 8 );
                                        }
                                        else
                                        {
                                            errflag = 0xFF;
                                    ...
                                    case HID_SET_PROTOCOL:
                                        ...
                                        {
                                            USBHS_HidProtocol[ 2 ] = (uint8_t)USBHS_SetupReqValue;
                                        }
                                        else
                                        {
                                            errflag = 0xFF;
                                    ...
                                    case HID_GET_IDLE:
                                        ...
                                        {
                                            USBHS_EP0_Buf[ 0 ] = USBHS_HidIdle[ 2 ];
                                            len = 1;
                                        }
                                        else
                                        {
                ...
                /* end-point 3 data out interrupt (Raw HID OUT) */
                case DEF_UEP3:
                    ...
                    if( len > 0 )
                    {
                        ...
                        raw_hid_receive( USBHS_EP3_RX_Buf, (uint8_t)len );
                    }
        ...
        else
        {
          /* data-in stage processing */
            switch ( endp_num )
            {
                ...
                /* end-point 3 data in interrupt (Raw HID IN) */
                case DEF_UEP3:
...
/*********************************************************************
 * @fn      USBHS_RawHID_Send
 *
 * @brief   Send Raw HID data through EP3 IN.
 *
 * @return  1: success, 0: failed
 */
uint8_t USBHS_RawHID_Send( uint8_t *data, uint8_t len )
{
    if( USBHS_DevEnumStatus == 0x00 )
    {
        return 0;
    }
    return USBHS_Endp_DataUp( DEF_UEP3, data, len, DEF_UEP_CPY_LOAD );
}
