...
#endif
        default: {
            /* CH585 移植（无 IAP 独立启动）：DataFlash 未初始化(0xFF)或读到未编译的
             * 协议模式时，原逻辑会写 IAP 标志并软复位 —— 但本工程不含 IAP，
             * 复位后仍是 0xFF，形成无限重启、USB 永不枚举。
             * 改为落定默认协议（USB）并写回 DataFlash，下次开机即正常。 */
            mode = bootloader_set_to_default_mode("Boot mode uninitialized or unsupported");
#ifdef USB_ENABLE
            if (mode == BOOTLOADER_BOOT_MODE_USB) {
                kbd_protocol_type = kbd_protocol_usb;
                ch582_set_protocol_interface(&ch582_protocol_usb);
...
#endif
            /* 连默认协议都不可用（功能配置异常）才保留复位行为 */
            PRINT("No usable protocol, will reboot.\n");
