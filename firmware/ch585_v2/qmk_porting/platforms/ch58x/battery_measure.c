...
__attribute__((noreturn)) __HIGH_CODE void battery_handle_critical()
{
    ...
    /* CH585: sys_safe_access_enable/disable 为 do{}while(0) 配对宏，
     * 使能窗口期内可连续写多个受保护寄存器（CH58x_sys.h 注释）。 */
    sys_safe_access_enable();
    R8_BAT_DET_CTRL = 0;
    ...
    sys_safe_access_enable();
    R8_SLP_POWER_CTRL = RB_RAM_RETENT_LV | 0x01;
    R16_POWER_PLAN = RB_PWR_PLAN_EN | RB_PWR_MUST_0;
    sys_safe_access_disable();
    __WFI();
    __nop();
    __nop();
